...
public class PA4 extends JFrame implements GLEventListener, KeyListener,
	...
	public void keyTyped(KeyEvent key) {
		...
		case '1':
			if (toggleLights) {
				if (light.all_lights.size() >= 	1) {
					light.all_lights.get(0).toggleLight();
				}
			}
			break;
		...
		case '2':
			if (toggleLights) {
				if (light.all_lights.size() >= 2) {
					light.all_lights.get(1).toggleLight();
				}
			}
			break;
		...
		case '3':
			if (toggleLights) {
				if (light.all_lights.size() >= 3) {
					light.all_lights.get(2).toggleLight();
				}
			}
			break;
		case 'R':
	...
	public void mouseDragged(final MouseEvent mouse) {
		if (this.rotate_world) {
			...
			if (magnitude > 0.0001) {
				...
				final float s = (float) Math.sin(0.99f * viewing_delta);
				final float c = (float) Math.cos(0.99f * viewing_delta);
	...
	void testOne() {
		...
		for (int k = 0; k < 5; ++k)
		{
			if (k == 0) {
				...
				n = box.getN();
				m = box.getM();
				for (Mesh3D boxMesh : box.all_meshes) {
	...
	void testThree() {
		...
		for (int k = 0; k < 4; ++k)
		{
			if (k == 0) {
				...
				n = box1.getN();
				m = box1.getM();
				for (Mesh3D boxMesh : box1.all_meshes) {
					drawObject(boxMesh, box1, n, m, view_vector, light, depthBuff);
				}
			} else if (k == 2) {
				n = box2.getN();
				m = box2.getM();
				for (Mesh3D boxMesh : box2.all_meshes) {
