...
public class Triangle {
	...
	// draw a line segment
	// EDITED TO USE FLOATING POINT ARITHMETIC: DDA ALGORITHM
	public static void drawLine(BufferedImage buff, DepthBuffer depthBuff,
		...
		if (pk.y >= 0 && pk.y < buff.getHeight() && pk.x >= 0
				&& pk.x < buff.getWidth()
				&& pk.z > depthBuff.depth_buffer[(int) pk.x][(int) pk.y]) {
			depthBuff.depth_buffer[(int) pk.x][(int) pk.y] = pk.z;
			drawPoint(buff, pk);
		}
		...
		if (p1.x == p2.x) { // Line is vertical
			for (int k = 0; k < steps; k++) {
				...
				if (y >= 0 && y < buff.getHeight() && x >= 0
						&& x < buff.getWidth()
						&& z > depthBuff.depth_buffer[(int) pk.x][(int) pk.y]) {
					depthBuff.depth_buffer[(int) pk.x][(int) pk.y] = pk.z;
